...
import {
  getKanbanPermissionRoles,
  deleteKanbanPermissionRole
} from '@/lib/api/services';
import { getError } from '@/lib/api/axios';
import type { KanbanRolePermission, KanbanSection } from '@/lib/api/types';
import {
  Table,
  TableBody,
  TableCell,
...
import KanbanPermissionRoleAddDialog from './kanban-permission-role-add-dialog';
import KanbanPermissionRoleEditDialog from './kanban-permission-role-edit-dialog';
...
interface KanbanPermissionRolesTabProps {
...
export default function KanbanPermissionRolesTab({
  sections
}: KanbanPermissionRolesTabProps) {
  ...
  // ── State ──────────────────────────────────────────────────────────────
  const [permissions, setPermissions] = useState<KanbanRolePermission[]>([]);
  ...
  const [editPermission, setEditPermission] =
    useState<KanbanRolePermission | null>(null);
  ...
  const fetchPermissions = useCallback(async () => {
    ...
    try {
      const response = await getKanbanPermissionRoles(companyId, kanbanId, {
        page,
        limit,
  ...
  const handleEditClick = (perm: KanbanRolePermission) => {
  ...
  const handleDeleteClick = (perm: KanbanRolePermission) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteKanbanPermissionRole(
        companyId,
        kanbanId,
        deletePermission.id
      );
  ...
  return (
    <>
                    ...
                    <div className='flex flex-col items-center gap-2'>
                      ...
                      <p className='text-muted-foreground'>
                        {searchQuery || selectedSectionId
                          ...
                          : 'No role permissions found. Add your first role.'}
                      </p>
                    </div>
                  </TableCell>
                </TableRow>
      ...
      {/* ── Add Role Permission Dialog ────────────────────────────────── */}
      <KanbanPermissionRoleAddDialog
        open={showAddDialog}
      ...
      <KanbanPermissionRoleEditDialog
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Remove Role Permission</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to remove permissions for{' '}
              <span className='font-semibold'>
                &quot;{deletePermission?.role?.name}&quot;
              </span>
